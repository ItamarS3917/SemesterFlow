...
const AppContent = () => {
  ...
  const Dashboard = () => {
    ...
    const isDashboardLoading =
      coursesLoading || assignmentsLoading || statsLoading || sessionsLoading;
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="retro-card p-5 flex flex-col relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-2 z-10">
              <span className="text-gray-400 text-xs font-mono font-bold uppercase tracking-wider">
                Phase Progress
              </span>
            </div>
            <h3 className="text-2xl font-bold text-white z-10 font-mono">
              {userStats.phaseProgress}%
            </h3>
            ...
            <div className="w-full bg-gray-700 h-3 mt-4 border-2 border-black rounded-full overflow-hidden z-10">
              <div
                className="bg-indigo-500 h-full border-r-2 border-black"
                style={{ width: `${userStats.phaseProgress}%` }}
              ></div>
            </div>
          </div>
          ...
          <div className="retro-card p-5 flex flex-col">
            ...
            <h3 className="text-2xl font-bold text-white font-mono">
              {userStats.weeklyHours}
              <span className="text-gray-500 text-lg">/</span>
              {userStats.weeklyTarget}h
            </h3>
            <div className="w-full bg-gray-700 h-3 mt-4 border-2 border-black rounded-full overflow-hidden">
              <div
                className="bg-orange-500 h-full border-r-2 border-black"
                style={{
                  width: `${Math.min(100, (userStats.weeklyHours / userStats.weeklyTarget) * 100)}%`,
                }}
              ></div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <ProcrastinationWidget onBreakPattern={handleBreakPattern} />
              ...
              <div className="space-y-4">
                {upcomingAssignments.map((assignment) => {
                  ...
                  return (
                    <div
                      key={assignment.id}
                      className="flex items-center p-4 bg-gray-800 border-2 border-gray-700 hover:border-indigo-500 rounded-lg transition-all hover:translate-x-1 group"
                    >
                      <div
                        className={`w-2 h-12 ${course?.color || 'bg-gray-600'} border-2 border-black mr-4 shadow-[2px_2px_0px_0px_#000]`}
                      ></div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span
                            className={`text-[10px] font-black uppercase px-2 py-0.5 border border-black shadow-[1px_1px_0px_0px_#000] bg-gray-700 text-gray-200`}
                          >
                            {course?.name}
                          </span>
                          {daysLeft <= 3 && (
                            <span className="text-[10px] font-bold bg-red-600 text-white px-2 py-0.5 border border-black">
                              DUE SOON
                            </span>
                          )}
                        </div>
                      ...
                      <div className="text-right font-mono">
                        <div
                          className={`text-lg font-bold ${daysLeft !== null && daysLeft <= 3 ? 'text-red-400' : 'text-gray-400'}`}
                        >
            ...
            <div className="retro-card bg-indigo-700 text-white p-6 flex flex-col justify-between relative overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] border-indigo-900">
              ...
              <div className="relative z-10">
                ...
                <p className="text-indigo-100 text-sm mb-8 font-medium">
                  Initialize study sequence. Track progress.
                </p>
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className="text-[10px] font-mono text-gray-600">
                        {course.hoursCompleted}h
                      </span>
                    </div>
                    <div className="w-full bg-gray-700 h-1.5 border border-black rounded-full overflow-hidden">
                      <div
                        className={`${course.color} h-full border-r border-black`}
                        style={{
                          width: `${Math.min(100, (course.hoursCompleted / course.totalHoursTarget) * 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
  ...
  return (
    <div className="min-h-screen flex bg-dots">
      ...
      <aside className="hidden md:flex w-72 bg-gray-900 border-r-2 border-black flex-col fixed h-full z-20 shadow-xl">
        <div className="p-6 border-b-2 border-black bg-gray-900">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 text-white flex items-center justify-center font-black text-2xl shadow-[3px_3px_0px_0px_#000] border-2 border-black">
              S
            </div>
      ...
      {/* Mobile Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 border-r-2 border-black transform transition-transform duration-200 ease-in-out md:hidden ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="p-6 flex items-center justify-between border-b-2 border-black">
          ...
          <button
            onClick={() => setMobileMenuOpen(false)}
            className="p-2 hover:bg-gray-800 rounded-lg border-2 border-transparent hover:border-black"
          >
      ...
      <main className="flex-1 md:ml-72 transition-all relative">
        ...
        <header className="sticky top-0 bg-gray-900/90 backdrop-blur-md border-b-2 border-black px-6 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-4">
            <button
              className="md:hidden p-2 border-2 border-gray-700 rounded-md hover:border-gray-500"
