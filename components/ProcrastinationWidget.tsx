...
export const ProcrastinationWidget: React.FC<ProcrastinationWidgetProps> = ({ onBreakPattern }) => {
  ...
  const completedOrStarted = assignments.filter(
    (a) => a.status !== AssignmentStatus.NOT_STARTED && a.startedAt
  );
  ...
  const handleGetAiAdvice = async () => {
    ...
    try {
      const historyData = completedOrStarted.map((a) => ({
        ...
        delayPercent:
          Math.round(
            ((new Date(a.startedAt!).getTime() - new Date(a.createdAt).getTime()) /
          ...
          <div className="space-y-4 mb-6">
            ...
            {dangerZoneAssignments.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="bg-gray-800 p-3 border-l-4 border-red-500 flex justify-between items-center group hover:bg-gray-700 transition-colors"
                >
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-[10px] px-1.5 py-0.5 border border-gray-600 uppercase font-bold text-gray-400`}
                      >
                        {course?.name}
                      </span>
        ...
        <div className="border-t border-gray-800 pt-4">
          {!aiTip ? (
            ...
            <div className="bg-indigo-900/20 border border-indigo-500/50 p-4 animate-fade-in relative">
              ...
              <p className="text-sm text-indigo-100 font-mono leading-relaxed italic">"{aiTip}"</p>
              <button
