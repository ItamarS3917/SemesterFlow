      ...
      complete: async (results) => {
        try {
          ...
          if (valid.length === 0) {
            setStatus({
              type: 'error',
              message: 'No valid assignments found. Check CSV headers: courseId, name, dueDate.',
            });
            setImporting(false);
            return;
          }
    ...
    <div className="retro-card p-4 border-dashed border-2 border-gray-600 bg-gray-900/50">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {status.message && (
            <span
              className={`text-xs font-bold font-mono flex items-center gap-1 ${status.type === 'success' ? 'text-green-400' : 'text-red-400'}`}
            >
              {status.type === 'success' ? (
