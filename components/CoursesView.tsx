...
import { Calendar, CheckCircle, BookOpen, History, Target, FileText } from 'lucide-react';
...
export const CoursesView: React.FC<CoursesViewProps> = ({ onNavigate }) => {
  ...
  const completedAssignmentsCount = courseAssignments.filter(
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row justify-between items-start mb-6 gap-4">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <div
                      className={`w-6 h-6 border-2 border-black ${selectedCourse.color} shadow-[2px_2px_0px_0px_#000]`}
                    ></div>
                    <h2
                      className={`text-3xl font-black text-white font-mono uppercase tracking-tighter`}
                    >
                      {selectedCourse.name}
                    </h2>
                  </div>
                  <div className="flex flex-wrap items-center gap-3 text-gray-400 mt-2">
                    ...
                    {selectedCourse.nextExamDate && (
                      <div className="flex items-center gap-2 bg-black px-3 py-1 border border-gray-700 text-xs font-mono font-bold uppercase text-red-400">
                        ...
                        <span>
                          Exam: {new Date(selectedCourse.nextExamDate).toLocaleDateString()}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-gray-900 border-2 border-black p-4 shadow-[4px_4px_0px_0px_#000]">
                ...
                <div className="w-full bg-gray-800 h-6 border-2 border-black relative">
                  ...
                  <div
                    className={`h-full ${selectedCourse.color} border-r-2 border-black transition-all duration-1000`}
          ...
          <div className="retro-card p-6">
            ...
            <div className="space-y-3">
              {courseAssignments.length === 0 ? (
                <p className="text-gray-500 text-center py-8 font-mono uppercase text-sm">
                  No tasks assigned.
                </p>
              ) : (
                courseAssignments
                  ...
                  .map((assignment) => (
                    <div
                      key={assignment.id}
                      className="flex items-center justify-between p-4 bg-gray-800 border-2 border-black shadow-[4px_4px_0px_0px_#000] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all"
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-4 h-4 border-2 border-black ${assignment.status === AssignmentStatus.COMPLETED ? 'bg-green-500' : assignment.status === AssignmentStatus.IN_PROGRESS ? 'bg-yellow-400' : 'bg-gray-700'}`}
                        ></div>
                        <div>
                          <div
                            className={`font-bold font-mono text-sm ${assignment.status === AssignmentStatus.COMPLETED ? 'text-gray-500 line-through' : 'text-white'}`}
                          >
                            {assignment.name}
                          </div>
              ...
              {courseSessions.length === 0 ? (
                ...
                courseSessions.slice(0, 5).map((session) => (
                  <div key={session.id} className="relative pl-6 border-l-2 border-gray-700">
                    <div
                      className={`absolute -left-[5px] top-1.5 w-2.5 h-2.5 border border-black ${selectedCourse.color}`}
                    ></div>
