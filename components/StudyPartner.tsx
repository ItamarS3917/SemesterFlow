...
export const StudyPartner = () => {
    ...
    const handleSaveToNotes = () => {
        ...
        if (true) { // addToKnowledge is always true in this context based on previous code logic
            updateCourse({
                ...selectedCourse,
                    ...
                    <div className="space-y-2">
                        {courses.map(c => (
                            <button
                                ...
                                className={`w-full text-left p-2 border-2 text-xs font-mono font-bold uppercase transition-all flex items-center justify-between ${selectedCourseId === c.id
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {(['SOCRATIC', 'STRICT', 'ENCOURAGING', 'HUMOROUS'] as TeacherPersonality[]).map(p => (
                                <button
                                    ...
                                    className={`p-2 text-[10px] border font-mono font-bold uppercase transition-all ${personality === p
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-2">
                            <button
                                ...
                                className={`p-3 text-xs border-2 font-mono font-bold uppercase transition-all flex items-center gap-2 ${mode === 'GUIDED_LEARNING'
                            ...
                            <button
                                ...
                                className={`p-3 text-xs border-2 font-mono font-bold uppercase transition-all flex items-center gap-2 ${mode === 'QUIZ_ME'
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-900 scrollbar-hide">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[90%] p-4 border-2 font-mono text-sm shadow-[4px_4px_0px_0px_#000] whitespace-pre-wrap ${msg.role === 'user'
